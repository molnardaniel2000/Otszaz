...
class Content {
    content(req, res) {
        ...
        const u = url_1.default.parse(req.url, true).query;
        let sorszam = parseInt(u.sorszam);
        ...
        let arucikk = u.arucikk;
        if (util_1.isUndefined(arucikk) || util_1.isNull(arucikk) || arucikk === "") {
            arucikk = "kefe";
        }
        ...
        if (util_1.isString(megoldas.eloszorVasaroltak(arucikk))) {
            res.write(`5. feladat\n${megoldas.eloszorVasaroltak(arucikk)}\n`);
        }
        else {
            res.write(`5. feladat\nAz első vásárlás sorszáma: ${megoldas.eloszorVasaroltak(arucikk)}\n`);
        }
        if (util_1.isString(megoldas.utoljaraVasaroltak(arucikk))) {
        ...
        fs_1.default.readFileSync("penztar.txt")
            .toString()
            .split("\r\n")
            .forEach(l => {
            res.write(l + "\n");
        ...
        fs_1.default.readFileSync("osszeg.txt")
            .toString()
            .split("\r\n")
            .forEach(l => {
            res.write(l + "\n");
